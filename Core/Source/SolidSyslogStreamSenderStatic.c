...
static bool StreamSender_IsValidConfig(const struct SolidSyslogStreamSenderConfig* config)
{
    ...
    if (config == NULL)
    {
        StreamSender_Report(SOLIDSYSLOG_BAD_CONFIG_FATAL_SEVERITY, SOLIDSYSLOG_CAT_BAD_CONFIG, STREAMSENDER_ERROR_NULL_CONFIG);
    }
    else if (config->Resolver == NULL)
    {
        StreamSender_Report(SOLIDSYSLOG_BAD_CONFIG_FATAL_SEVERITY, SOLIDSYSLOG_CAT_BAD_CONFIG, STREAMSENDER_ERROR_NULL_RESOLVER);
    }
    else if (config->Stream == NULL)
    {
        StreamSender_Report(SOLIDSYSLOG_BAD_CONFIG_FATAL_SEVERITY, SOLIDSYSLOG_CAT_BAD_CONFIG, STREAMSENDER_ERROR_NULL_STREAM);
    }
    else if (config->Address == NULL)
    {
        StreamSender_Report(SOLIDSYSLOG_BAD_CONFIG_FATAL_SEVERITY, SOLIDSYSLOG_CAT_BAD_CONFIG, STREAMSENDER_ERROR_NULL_ADDRESS);
    }
    else
    {
        valid = true;
