...
| `SolidSyslogLwipRawResolver.h` | System setup code on **any target using lwIP Raw API** that resolves literal numeric IPv4 hosts (no DNS) | `SolidSyslogLwipRawResolver_Create(void)`, `_Destroy(base)` — Resolve delegates to lwIP's `ipaddr_aton` and rejects anything the parser rejects (DNS names, alphabetic input, empty string); transport is ignored. lwIP's parser is lenient about notation (Berkeley-style `"10.0.2"` is accepted as numeric IPv4). DNS-name resolution will land as the sibling `SolidSyslogLwipRawDnsResolver` (S28.07). Instance struct lives in a library-internal static pool (E11). Pool-exhaustion fallback is the shared `SolidSyslogNullResolver`. |
