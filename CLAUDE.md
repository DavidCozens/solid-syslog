...
| `SolidSyslogAddress.h` | Any code holding an address handle | Opaque `struct SolidSyslogAddress` forward declaration only — the destination slot a Resolver writes into and a Datagram / Stream reads from. Handles come from the per-platform `SolidSyslog{Posix,Winsock,FreeRtos}Address_Create()` factories below. |
...
| `SolidSyslogFreeRtosAddress.h` | System setup code on FreeRTOS targets wiring a UDP / TCP destination | `SolidSyslogFreeRtosAddress_Create(void)`, `_Destroy(base)` (wraps `struct freertos_sockaddr`). Same pool semantics as `SolidSyslogPosixAddress`. |
| `SolidSyslogFreeRtosAddressErrors.h` | Any code installing an error handler that wants to react to FreeRtosAddress-specific events (pointer-identity match on `FreeRtosAddressErrorSource`, switch on `enum SolidSyslogFreeRtosAddressErrors`) | `enum SolidSyslogFreeRtosAddressErrors` (`FREERTOSADDRESS_ERROR_*` codes + `FREERTOSADDRESS_ERROR_MAX` bookend), `extern const struct SolidSyslogErrorSource FreeRtosAddressErrorSource`. Integrators ignore if not handling errors per source. |
...
| `SolidSyslogFreeRtosResolver.h` | System setup code on FreeRTOS targets pinned to a hardcoded IPv4 destination (no DNS) | `SolidSyslogFreeRtosResolver_Create(ipv4Octets)`, `_Destroy(base)` (ignores host and transport at Resolve time; port is taken from each call). Instance struct lives in a library-internal static pool (E11). Pool-exhaustion fallback is the shared `SolidSyslogNullResolver`. |
| `SolidSyslogFreeRtosResolverErrors.h` | Any code installing an error handler that wants to react to FreeRtosResolver-specific events (pointer-identity match on `FreeRtosResolverErrorSource`, switch on `enum SolidSyslogFreeRtosResolverErrors`) | `enum SolidSyslogFreeRtosResolverErrors` (`FREERTOSRESOLVER_ERROR_*` codes + `FREERTOSRESOLVER_ERROR_MAX` bookend), `extern const struct SolidSyslogErrorSource FreeRtosResolverErrorSource`. Integrators ignore if not handling errors per source. |
...
| `SolidSyslogFreeRtosDatagramErrors.h` | Any code installing an error handler that wants to react to FreeRtosDatagram-specific events (pointer-identity match on `FreeRtosDatagramErrorSource`, switch on `enum SolidSyslogFreeRtosDatagramErrors`) | `enum SolidSyslogFreeRtosDatagramErrors` (`FREERTOSDATAGRAM_ERROR_*` codes + `FREERTOSDATAGRAM_ERROR_MAX` bookend), `extern const struct SolidSyslogErrorSource FreeRtosDatagramErrorSource`. Integrators ignore if not handling errors per source. |
...
| `SolidSyslogFreeRtosTcpStreamErrors.h` | Any code installing an error handler that wants to react to FreeRtosTcpStream-specific events (pointer-identity match on `FreeRtosTcpStreamErrorSource`, switch on `enum SolidSyslogFreeRtosTcpStreamErrors`) | `enum SolidSyslogFreeRtosTcpStreamErrors` (`FREERTOSTCPSTREAM_ERROR_*` codes + `FREERTOSTCPSTREAM_ERROR_MAX` bookend), `extern const struct SolidSyslogErrorSource FreeRtosTcpStreamErrorSource`. Integrators ignore if not handling errors per source. |
...
| `SolidSyslogUdpSender.h` | System setup code using UDP transport | `SolidSyslogUdpSenderConfig` (resolver, datagram, address, endpoint, endpointVersion — the integrator supplies one platform Address handle per sender from `SolidSyslog{Posix,Winsock,FreeRtos}Address_Create`), `SolidSyslogUdpSender_Create(config)`, `_Destroy(sender)`. Instance struct lives in a library-internal static pool (E11); `_Destroy` takes the handle returned by `_Create`. Pool-exhaustion and bad-config (including NULL Address) fallback is the shared `SolidSyslogNullSender`. |
...
| `SolidSyslogStreamSender.h` | System setup code using octet-framed transport (RFC 6587) over any Stream — `SolidSyslogPosixTcpStream` (POSIX TCP), `SolidSyslogWinsockTcpStream` (Windows TCP), `SolidSyslogFreeRtosTcpStream` (FreeRTOS-Plus-TCP), `SolidSyslogTlsStream` (TLS; OpenSSL reference integration), `SolidSyslogMbedTlsStream` (TLS; Mbed TLS reference integration for embedded targets), or a caller-supplied Stream backend | `SolidSyslogStreamSenderConfig` (resolver, stream, address, endpoint, endpointVersion — the integrator supplies one platform Address handle per sender from `SolidSyslog{Posix,Winsock,FreeRtos}Address_Create`), `SolidSyslogStreamSender_Create(config)`, `_Destroy(base)`. Instance struct lives in a library-internal static pool (E11); `_Destroy` takes the handle returned by `_Create`. Pool-exhaustion and bad-config fallback is the shared `SolidSyslogNullSender`. |
