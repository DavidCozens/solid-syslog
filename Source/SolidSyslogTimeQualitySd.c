...
static void Format(struct SolidSyslogStructuredData* self, struct SolidSyslogFormatter* formatter)
{
    ...
    SolidSyslogFormatter_BoundedString(formatter, "[timeQuality", 12);
    FormatBoolParam(formatter, " tzKnown", 8, q.tzKnown);
    FormatBoolParam(formatter, " isSynced", 9, q.isSynced);
...
static void FormatSyncAccuracy(struct SolidSyslogFormatter* formatter, uint32_t value)
{
    ...
    SolidSyslogFormatter_BoundedString(formatter, " syncAccuracy=\"", 15);
