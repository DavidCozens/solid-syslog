...
|------|------:|-----------|---------|-----------|-------|
| **enum constant** | 252 → 0 *(landed in S10.07)* | `SolidSyslogPrival.h` (128 — `SOLIDSYSLOG_FACILITY_*`, `SOLIDSYSLOG_SEVERITY_*`), `SolidSyslogTransport.h` (24), `SolidSyslogBlockStore.h` (12) | **Fix — landed in S10.07** | `docs/NAMING.md` already specifies `SolidSyslogClass_Constant` form (e.g. `SolidSyslogSeverity_Emergency`). Current SCREAMING_SNAKE is the older convention. Renamed across the tree in five per-enum slices on `feat/s10-07-enum-rename`. | **S10.07** |
...
|---------|------:|----------------|
| **Fix — landed in S10.07** — enum-constant + enum-tag rename | 260 → 0 | SCREAMING_SNAKE enum constants (252) + `SolidSyslog_<X>` enum tags (8) |
